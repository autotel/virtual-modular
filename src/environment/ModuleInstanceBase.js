...
module.exports = function (properties, environment) {
  ...
  var currentFrameEventMessages = [];
  var exchangeImmediate = false;
  function exchangeStep() {
    outputs.forEach(function (outputModule) {
      for (var n in currentFrameEventMessages) {
        var eventMessage = currentFrameEventMessages[n].clone();
        currentFrameEventMessages[n] = eventMessage;
        outputModule.messageReceived({ eventMessage: eventMessage.clone(), origin: self });
        self.handle('> message', { origin: self, destination: outputModule, val: eventMessage });
      }
      if (outputModule.messageStream) outputModule.messageStream({ messages: currentFrameEventMessages });
    });
    exchangeImmediate = false;
    currentFrameEventMessages = [];
  }
  ...
  this.output = function (eventMessage, overrideMute) {
    if ((!self.mute) || overrideMute) {
      currentFrameEventMessages.push(eventMessage);
      if (!exchangeImmediate) exchangeImmediate = setImmediate(exchangeStep);
    }
  }
